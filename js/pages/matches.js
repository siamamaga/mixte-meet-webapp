...
const MatchesPage = (() => {
  ...
  function render() {
    const totalUnread = DEMO_MATCHES.reduce((a, m) => a + m.unread, 0);
    ...
    document.getElementById('page-matches').innerHTML = `
      <div class="page-header">
        ...
        <span style="font-size:13px;color:var(--muted);">${DEMO_MATCHES.length} matchs</span>
      </div>
        ...
        ${DEMO_MATCHES.map(m => `
          <div class="match-item" onclick="ChatPage.open(${JSON.stringify(m).replace(/"/g,'&quot;')})">
            <div class="match-avatar">
              <div class="match-avatar-emoji" style="background:linear-gradient(135deg,#2D1F38,var(--pink));display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:32px;">${m.emoji}</div>
              ${m.online ? '<div class="match-online-dot"></div>' : ''}
            </div>
            <div class="match-info">
              <div class="match-name">${m.first_name} <span style="font-size:12px;color:var(--muted);font-weight:400;">${Utils.countryFlag(m.country_code)}</span></div>
              <div class="match-preview ${m.unread ? 'style="color:rgba(255,255,255,0.8);font-weight:500;"' : ''}">${m.last_message}</div>
            </div>
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0;">
              <span class="match-time">${Utils.timeAgo(m.last_message_at)}</span>
